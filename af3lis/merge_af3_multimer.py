# ---------------------------------------------------------------------------
# VENDORED VERBATIM from Mau's bwHelix AF3 pipeline toolkit
#   original filename: merge_af3_multimer_v6.py  (2026-08-21)
#   received as AF3_pipeline_toolkit.zip, personal communication, Aug 2026
#
# Do NOT edit below this header. af3lis drives this script as a subprocess in
# single-pair mode so the body stays diffable against future versions:
#
#   python -m af3lis.merge_af3_multimer <monoA.json> <monoB.json> <out.json> \
#       --name <A>___<B> --seeds 1 2 3 --quiet
#
# Why it matters: AF3's data pipeline aligns every chain in each input JSON, so
# a pairwise grid re-aligns the same sequence once per pair it appears in (2x
# for 1x1000, 4x for 2x500, 28x for 20x50). This merges separately-aligned
# MONOMER _data.json files into a multimer input, so each unique chain is
# aligned exactly once. The approach is sound because in AF3 each chain's
# unpairedMsa/pairedMsa/templates are per-chain search results; the cross-chain
# pairing happens later, at featurisation.
# ---------------------------------------------------------------------------
#!/usr/bin/env python3
"""
...
def update_chain_id(sequence_data: Dict[str, Any], new_chain_id: str, verbose: bool = True) -> Dict[str, Any]:
    """Update the chain ID in a sequence entry."""
    # Deep copy to avoid modifying original
    updated = json.loads(json.dumps(sequence_data))
    
    # Update protein chain ID if it exists
    if 'protein' in updated and 'id' in updated['protein']:
        updated['protein']['id'] = new_chain_id
        if verbose:
            print(f"  Updated protein chain ID to: {new_chain_id}")
...
def perform_merge(
    ...
    """
    Perform the merge of two AlphaFold 3 data dictionaries in memory.
    ...
    total_chains = len(data1.get('sequences', [])) + len(data2.get('sequences', []))
    ...
    if verbose:
        print(f"  Merging with chain IDs: {chain_ids[:total_chains]}")
    
    # Process sequences from first file
    chain_idx = 0
    for seq in data1.get('sequences', []):
        updated_seq = update_chain_id(seq, chain_ids[chain_idx], verbose=False)
        ...
        if verbose and 'protein' in updated_seq:
            ...
            print(f"    Chain {chain_ids[chain_idx]}: {seq_len} residues from {file1_path.name}")
        
        chain_idx += 1
    
    # Process sequences from second file
    for seq in data2.get('sequences', []):
        updated_seq = update_chain_id(seq, chain_ids[chain_idx], verbose=False)
        merged_data['sequences'].append(updated_seq)
        
        if verbose and 'protein' in updated_seq:
            seq_len = len(updated_seq['protein'].get('sequence', ''))
            print(f"    Chain {chain_ids[chain_idx]}: {seq_len} residues from {file2_path.name}")
    ...
    if custom_seeds is not None:
        ...
        seeds1 = set(data1.get('modelSeeds', []))
        seeds2 = set(data2.get('modelSeeds', []))
        all_seeds = sorted(seeds1.union(seeds2))
        if all_seeds:
            merged_data['modelSeeds'] = all_seeds
    
    # Handle bonded atom pairs
    bonds1 = data1.get('bondedAtomPairs', [])
    bonds2 = data2.get('bondedAtomPairs', [])
    if bonds1 or bonds2:
        merged_data['bondedAtomPairs'] = bonds1 + bonds2
    
    # Merge user CCD
    ccd1 = data1.get('userCCD', [])
    ccd2 = data2.get('userCCD', [])
    if ccd1 or ccd2:
        ccd_dict = {}
...
def merge_single_pair(
    ...
    # Generate multimer name if not provided
    if multimer_name is None:
        if job_number is not None:
            multimer_name = f"job_{job_number}_{name_a}_{name_b}"
        else:
            multimer_name = f"{name_a}_{name_b}_multimer"
        ...
        for j, file2 in enumerate(files2):
            ...
            for seed in seeds_to_expand:
                ...
                if output_naming == 'names':
                    output_name = f"{name_a}_{name_b}"
                elif output_naming == 'numbers':
                    output_name = f"job_{job_number}"
                else:  # 'both'
                    output_name = f"job_{job_number}_{name_a}_{name_b}"
...
def convert_to_simple_format(merged_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    simplified = {
        ...
        'modelSeeds': merged_data.get('modelSeeds', []),
        'sequences': []
    }
    
    for seq in merged_data.get('sequences', []):
        if 'protein' in seq:
            protein = seq['protein']
            simplified_seq = {
                'proteinChain': {

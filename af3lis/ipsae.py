...
if af3:
    ...
    # Check for dimension mismatch between token_mask and PAE matrix
    pae_size = pae_matrix_af3.shape[0]
    token_size = len(token_array)
    if token_size != pae_size:
        print(f"  Warning: token_mask length ({token_size}) != PAE matrix size ({pae_size}). Adjusting...")
        if token_size > pae_size:
            # Truncate token_array to match PAE matrix
            token_array = token_array[:pae_size]
        else:
            # Pad token_array with zeros
            token_array = np.pad(token_array, (0, pae_size - token_size), mode='constant', constant_values=0)

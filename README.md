...
and writes `env/activate.sh`. Every later invocation — including each SLURM
job — sources that file and gets the same interpreter. There is nothing to
activate by hand and nothing in your shell profile to break.

```
env/af3lis.yml        the spec: python 3.11, numpy, scipy, pandas,
                      matplotlib-base, pyyaml, pytest
$HOME/.af3lis/env     the env      (default prefix; override AF3LIS_ENV_PREFIX)
env/activate.sh       the loader   (gitignored; generated by setup)
```

**Why the env is under `$HOME` and not inside the clone.** Conda populates a
prefix by hardlinking out of its package cache. On parallel filesystems — GPFS
on bwHelix — `$HOME` and a `/work` workspace are separate *filesets*, and
hardlinks across filesets fail: conda dies mid-transaction and cannot even
unlink its own half-written files. Putting the prefix next to the package cache
under `$HOME` avoids that entirely. `bootstrap.sh` also hardlink-tests the
location up front and falls back to `--copy` on a filesystem with no hardlink
support at all, so the failure mode is a clear message rather than 4,000 lines
of rollback warnings.

Overrides, in precedence order: `AF3LIS_PYTHON` (use your own interpreter) →
`AF3LIS_ENV_PREFIX` (build somewhere else) → `$HOME/.af3lis/env` → whatever
`python3` resolves to. If you already keep a conda env with the analysis stack,
point `AF3LIS_PYTHON` at it and skip the build.
